...
Both solutions have their own challenges. Let’s start with the first approach.

### Using a Prebuilt Component

Most libraries allow customisation through themes. However, like with good ol’ Bootstrap, there are always recognisable elements that are either unmodifiable or very difficult to change.

If you're like me, you often struggle with the embedded component styling. Overriding styles with hacky solutions and dealing with missing features in the component API. You strive to create the best user experience, but the component library holds you back.

<aside>
  👉 It's never a good sign when you battle with a library. Component libraries should save time and effort, not
  increase it.
</aside>

I gave Material UI a shot to build the mentioned component. Here’s how it turned out:
...
While the code is acceptable, I’m working against the styling of the component, making it harder to maintain.

</details>

---

You can still recognise Material UI, especially the signature ripple effect indicating a user press. We could try to remove these things, but again, we’d be working against the library.

<aside>
👉 Depending on the type of application you are building, not having full control over the styling might be beneficial. For company internal applications or proofs of concept, prebuilt libraries can be great. However, for public-facing applications, you probably want more control.
...
Alright, so you decide to build the component from scratch. You quickly realize—or already know—that the native [`select`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/select) element [falls short in terms of styling](https://stackoverflow.com/questions/1895476/how-do-i-style-a-select-dropdown-with-only-css), so you need to create something custom.
...
These are interaction patterns not provided—or not extensible enough—by the web platform.

An example is the Combobox. This common component is not provided by browsers in any way. People have come to expect this pattern when dealing with large datasets and long lists of options. WIA (Web Accessibility Initiative) has described the [Combobox Pattern](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) in detail.
...
Looking at the code, the end result is pretty similar to the “from scratch” example. We focus mainly on UI elements and business logic. The component remains super readable, but we also gain some additional features.
...
<aside>
👉 While these libraries help with describing the component using ARIA roles, ensure that custom or hacky solutions maintain correct ARIA roles. *“No ARIA is better than bad ARIA”.* <sup>[[3]](#sources-3)</sup>
