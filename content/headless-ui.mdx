...
Both options have their own set of challenges. Let’s start by discussing the first one.
...
Looking at the code, the end result is pretty similar to the “from scratch” example. We focus mainly on UI elements and keeping track of the selected person. The component remains super readable, but we also gained some additional features.
